...
ggplot(df_ib_a) +
  geom_sf(fill= "NA")+
  geom_sf(data = lav_df_hh_int_a, aes(color = Anopheles_Caught), size = 1, alpha = 0.5)+
...
hh_sum_agugu <- st_transform(hh_sum_agugu, 32631)


ag_breeding_hull <- lav_df_hh_int_a_h %>%
  st_union() %>%
  st_convex_hull()

##Extract households within hull of Agugu breeding sites
households_in_hull_ag <- hh_sum_agugu[st_within(hh_sum_agugu,
                                                           ag_breeding_hull, sparse = FALSE), ]
...
hh_members_ag_dry <- ib_all_drydata %>%
  ...
  semi_join(households_in_hull_ag %>% st_drop_geometry() %>% dplyr::select(sn),
            by = "sn") %>%
...
hh_members_ch_dry <- ib_all_drydata %>%
  # keep only rows whose sn exists in hh_sum_agugu
  semi_join(households_in_hull_chl %>% st_drop_geometry() %>% dplyr::select(sn),
            by = "sn") %>%
  # keep just the columns you want
  dplyr::select(sn, hl4, hl5)
